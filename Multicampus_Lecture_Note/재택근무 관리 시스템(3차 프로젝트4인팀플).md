...
passport도 미들웨어임, 사용자 인증을 구현해 줄 것임
...
그 전에 npm install passport passport-local 설치
...
![image-20200330232625018](C:\Users\KAUstar\AppData\Roaming\Typora\typora-user-images\image-20200330232625018.png)
...
기억해야 돼! passport.authenticate()는 username(여기서는 email)과 password를 찾아보도록 설정 되어 있다!!!!!



globalRouter.post(routes.join,postJoin,postLogin);

즉, 이 postJoin은 이메일,패스워드 등 정보들을 받아서 사용자를 가입시키고(postJoin은 미들웨어이다 현재) next()가 호출되어서 다음 것으로 가고, 같은 정보를 전달하게 되는데 

다음 것인 postLogin은 받은 정보를 가지고 사용자를 로그인 시켜주는 거야!!!





middlewares.js파일의 res.locals.user를 req.user; 로 수정하자

Why??? passport가 사용자를 로그인 시킬 때, passport는 쿠키나 serialize, deserialize 등의 기능을 다 지원해줌은 물론이고, user가 담긴 object를 요청(request)에도 올려줄 거야!!!



테스트 해보기 전에 한가지 더!!!!

app.js 파일로 가서 passport를 가져오기(import), 사용(use)해야 한다.
...
app.use(passport.session());  //이건 session이라는 것을 저장시켜 줄 것임
...
위에서 실행된 cookieParser로부터 쿠키가 쭉 내려와서 passport는 초기화(initialize)되고,

그 다음엔 passport가 제 스스로 쿠키를 들여다봐서, 그 쿠키 정보에 해당하는 사용자를 찾아 줄 것임

**그리고, passport는 자기가 찾은 사용자를 요청(request)의 object, 즉 req.user로 만들어준다**

그러면, 그 user object를 templete에 추가시켜 줄 수가 있다.

예를들어 header.pug 파일로 가면 이 사용자가 인증이 되었나 안되었나 확인하는 부분이 문제없이 작동하겠지?



res.locals.user= req.user || {};

user가 존재하거나, 존재하지 않다면 비어있는 object를 주도록!!!!



app.js 파일에서 import "./passport" 해주기!!!



우리 npm install session / npm install express-session 다해주자

session 데이터는 쿠키 자체에 저장되지 않습니다.





**secret이란, 무작위 문자열로서 , 쿠키에 들어있는 session ID를 암호화하기 위한 것이다!!**

예를 들어, session ID를 전송할 때, 실제로 그 ID 값 그대로를 보내는 것이 아니다

실제로는 암호화된 상태로 보내게 된다. 뭘? 서버에서 session ID를 클라이언트로!!!



app.use(session({

secret : process.env.COOKIE_SECRET       // .env 파일에 암호화시킬 수 있는 키 설정해두기

resave: true // 세션을 강제로 저장합니다.

saveUninitialized : false  //초기화 되지 않은 세션을 저장소에 저장합니다.
...
세션 ID 까지 발급된걸 브라우저를 통해서 봤으면 알 수 있는게

원래 내 유저 ID는 1인데 COOKIE_SECRET으로 암호화됬음



그리고 내가 웹브라우저를 새로고침 할 때마다, 즉 이정보를 웹서버로 전송할 때마다

서버에서는 passport 인증과정을 호출하고

passport.js에서 deserializerUser 를 통해 내가 어느 사용자인지 식별할 수 있게 된다.



이제 누가 무슨 요청을 하고 있는지 알 수 있지. 

req.user는 이 어플리케이션 어디든 있을 수 있다. 왜냐? middleware.js파일에 로커변수 블라블라 설정을 해놨기때문에, 또한 쿠키가 있기 때문에



이 쿠키는 express로 보내지고 있어



express는 session을 이용함으로써 쿠키를 그 손에 쥘 수 있게 된것임



그리고 passport를 통해서, 우린 session을 이용하는데, 즉 session이 가진 쿠키를 이용한다는걸 의미한다.

그리고 그 passport를 통해서 deserialize를 진행하는 것이지



app.use(passport.session()); => session은 쿠키를 해독하거든(암호화되어있는 ID를 해독)

그리고 passport.deserializeUser로 넘어가 함수 실행

deserialize로 사용자를 식별하게 되면,

passport는 방금 찾은 그 사용자를 , middleware나 routes의 request(요청) object에 할당하게 된다.

그래서, 이제 어느 route에서든 로그인한 사용자가 누구인지 체크할 수 있게 된다.





내가 서버를 다시 시작할 때마다 session정보를 잃게 되는 것은 별로 좋지 못하다.

지금 우리는, session 정보, 쿠키 정보들을 메모리에 저장하고 있기 때문



이건 바꿔야 함, 왜냐면 난 session이 유지되기를 바라거든. 심지어 내가 서버를 잃거나 바꾸는 일이 있다고 해도 서버는 기억해야 한다. 어느 사용자가 어느 쿠키를 가지고 있는 지를
...
유덕경이라는 사람이 서버에 접속을 해서 서버로부터 파란 쿠키를 받았어

그러면 서버 입장에서는 유덕경이라는 사람한테 파란쿠키를 줬다는 것을 기억하고 있어야겠지?



지금 이렇게 세션을 잃어버렸다는 상황은, 서버가 재시작되면 서버입장에서는 누구한테 어떤 쿠키를 줬는지 알 수 없게 된느 것과 마찬가지임

<hr>

**그래서 mongoDB, 즉 데이터베이스를 사용해서 세션을 저장할 것임**

**connect-mongo**라는 걸 이용할 것임(이걸로 저장소를 생성할 것임)



connect-mongo(mongoDB와 연결하기)를 써서, session에게 데이터를 MongoStore라는 저장소에 저장하라고 알려주게 할 것이다.

npm install connect-mongo



app.js파일에 import 해주고

쿠키저장소를 만들어보자

그리고 이것은 session object를 필요로 함으로 MongoStore()함수 안에 써주기

const cookieStore = MongoStore(session)



store: new cookieStore({}) 를 app.use(session({}))에 추가시켜주기

즉, 느낌이 뭐냐면 말이야

session ID를 담고 있는 쿠키저장소를 mongoDB 저장소에 연결시켜주는 느낌



mongoDB와 연결을 시켜주는 역할은 mongoose가 하니까 import mongoose 해오기





이제는 로그인이 된 사용자는 Join화면으로 접근을 못하게 해보자 등등

이럴 떄마다 만들어야 하는게 middleware이다





middlewares.js 파일에다가 미들웨어를 작성해보자

export const Public = (req,res,next)=>{
...
이제 globalRouter 로 가서 미들웨어 추가해줘 봐



만약 사용자가 로그인 상태라면, Join과 Login화면을 보게 하고 싶지 않아

Join이랑 Login은 오로지 로그아웃된 사용자만 이용 가능함



사용자가 로그인되어 있으면, 이런 것들을 보게 하고 싶지 않다.





이제 github 로그인을 구현 할 것이다.
...
일단,우리는 사용자를 깃헙 페이지로 보낼 것이다.

어떤, 특별한 URL을 이용해서



사용자를 거기로 보내면, 깃헙이 "이 사람들(application= 내가 만든 웹앱 사이트)"에게 너의 정ㅂ를 줘도 괜찮으십니까? 내가 괜찮다고 하면~



사용자를 다시 우리 웹 사이트로 돌려보내고, 그 사용자의 정보도 같이 줄 것임



passport.js 파일에서 github strategy를 가져 오도록 하자





https://console.developers.google.com/apis/credentials?project=oauhpractice
...
localtunnel 은 우리의 로컬 서버에 https 터널을 만들어준다.